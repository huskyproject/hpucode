...
static char * invalidExt[] =
{
    "*.mo?", "*.tu?", "*.we?", "*.th?", "*.fr?", "*.sa?", "*.su?", "*.pkt", "*.tic"
};
...
int isReady(UUEFile * uuc)
{
    ...
    if(!uuc)
    {
        return 0;      /* Check pointers before use */
    }
...
int CompareUUEFile(char * p_e1, char * p_e2)
{
    ...
    if(!p_e1 || !p_e2)
    {
        return p_e1 > p_e2 ? 1 : p_e1 == p_e2 ? 0 : -1;
    }
...
void AddPart(UUEFile * uuc, char * msgBody, char * uuepart, int section, int slen)
{
    if(!uuc || !uuepart)
    {
        return;                  /* Check pointers before use */
    }
...
void MakeFile(UUEFile * uuc)
{
    ...
    if(dupeDetection(msg) == 1)
    {
        ...
        if(!fexist(fname))
        {
            ...
            if(out)
            {
                ...
                {
                    w_log(LL_INFO,
                          "file: %15s has %d complete sections is saved",
                          uuc->m_fname,
                          uuc->m_nSections);
                    MakeTicFile(uuc);
                    nodel = 0;
    ...
    if((!nodel) && (nDelMsg || nCutMsg))
    {
        ...
        {
            if((nMaxDeleted > 0) &&
               (toBeDeleted[nMaxDeleted - 1].nDelMsg == uuc->toBeDeleted[i].nDelMsg))
            {
